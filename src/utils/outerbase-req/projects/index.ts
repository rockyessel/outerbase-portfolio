...
export const getAllProjects = async (pageOffset: number = 0) => {
    const { data } = await axios.get(`${baseURL}/projects/all?pageOffset=${pageOffset}`);
    return data;
};

export const createProject = async (project: ProjectItem) => {
  const { data } = await axios.post(`${baseURL}/projects/create`, {...project})
  return data
}
...
export const getProjectBySlug = async (slug:string) => {
  const { data } = await axios.get(`${baseURL}/projects/data/slug?slug=${slug}`);
  return data?.response?.items?.[0];;
};

export const updateProject = async (project: ProjectItem, projectId:string) => {
  const { data } = await axios.put(`${baseURL}/projects/update?id=${projectId}`, { ...project })
  return data
}